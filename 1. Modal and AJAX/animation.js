window.onload = () => {
  ...
  let alive = positions.map((cell) => parseInt(cell ,10))

// Trying to make a circular animation
  // circle = (startPos) => {
  //   let s = startPos;
  //   let positions = [ 50, 51, 1, 49, 50, 51, 1, 49]
  ...
  renderField = (alive) => {
    ...
    alive.map((cell) => field[cell] = `<div key=${cell} class="alive" id=${cell}/></div>` )

    document.getElementById('body').innerHTML = field.join('')
  }

  checkAlignmentX = (cell, newPos) => Math.floor(cell / 50) === Math.floor(newPos / 50)

  checkAlignmentY = (cell, newPos) => (cell - newPos) % 50 === 0

  checkDiagonal = (cell, newPos) => (newPos % 50) - (cell % 50) > 0
